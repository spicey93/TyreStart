...
def create_sale(customer_id, status, date, items, ref_invoice=None):
    """Insert a sale with its line items. Its number is autogenerated from the
    status. ``ref_invoice`` links a Credit Note to the invoice it credits. Returns
    the new sale id."""
    date = daterange.to_iso(date)
    with get_connection() as conn:
        cursor = conn.execute(
            "INSERT INTO sales (customer_id, status, ref_invoice, date, created_at) "
            "VALUES (?, ?, ?, ?, ?)",
            (customer_id, status, ref_invoice, date, _now()),
        )
...
def update_sale(sale_id, customer_id, status, date, items, ref_invoice=None):
    ...
    with get_connection() as conn:
        conn.execute(
            "UPDATE sales SET customer_id = ?, status = ?, ref_invoice = ?, date = ? "
            "WHERE id = ?",
            (customer_id, status, ref_invoice, date, sale_id),
        )
        _assign_number(conn, sale_id, status)
...
def get_sale(sale_id):
    ...
    with get_connection() as conn:
        return conn.execute(
            ...
            "s.quote_no, s.order_no, s.invoice_no, s.credit_no, s.ref_invoice, s.date, "
