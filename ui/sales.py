...
class SalesMixin:
    ...
    def show_sale_form(self, sale=None, prefill_product=None, prefill_service=None,
        ...
        # Single-row details panel: Customer, Status, Reference and Date side by side
        # (less horizontal empty space than the old two-column layout).
        ...
        new_customer_button = {"widget": None}  # filled in by render_customer()
        ...
        def render_customer():
            ...
            if customer_state["id"] is not None:
                ...
                new_customer_button["widget"] = new_button
                ttk.Button(
                    cust_frame, text="🔍", width=3,
        ...
        # Remaining header fields laid out along the same row, to the right.
        ttk.Label(head, text="Status:").grid(row=0, column=2, sticky="w", pady=6, padx=(30, 10))
        ttk.Combobox(
            head, state="readonly", width=14, textvariable=status_var,
            values=list(sale_db.STATUSES),
        ).grid(row=0, column=3, sticky="w", pady=6)

        ttk.Label(head, text="Reference:").grid(row=0, column=4, sticky="w", pady=6, padx=(30, 10))
        # Autogenerated per status (Q…/O…/INV…); not user-editable.
        ttk.Entry(head, textvariable=reference_var, width=16, state="readonly").grid(
            row=0, column=5, sticky="w", pady=6
        )

        ttk.Label(head, text="Date:").grid(row=0, column=6, sticky="w", pady=6, padx=(30, 10))
        ttk.Entry(head, textvariable=date_var, width=12).grid(row=0, column=7, sticky="w", pady=6)

        # --- Vehicle panel: VRM look-up that links a vehicle to this sale ---
        vehicle_state = {"id": None}
        veh = ttk.LabelFrame(self.container, text="Vehicle", padding=12)
        veh.pack(anchor="w", fill="x", pady=(12, 0))
        vrm_var = tk.StringVar()
        ttk.Label(veh, text="VRM:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        vrm_entry = ttk.Entry(veh, textvariable=vrm_var, width=12,
                              font=("Consolas", 12, "bold"))
        vrm_entry.grid(row=0, column=1, sticky="w")
        veh_lookup_btn = ttk.Button(veh, text="Look up", command=lambda: lookup_vehicle())
        veh_lookup_btn.grid(row=0, column=2, sticky="w", padx=(8, 0))
        veh_info = ttk.Label(veh, text="No vehicle linked.", foreground="#C9A227")
        veh_info.grid(row=0, column=3, sticky="w", padx=(20, 0))
        veh_clear_btn = ttk.Button(veh, text="✕", width=3, command=lambda: clear_vehicle())

        def set_vehicle(row, summary):
            vehicle_state["id"] = row["id"] if row else None
            veh_info.config(text=summary, foreground="")
            veh_clear_btn.grid(row=0, column=4, sticky="w", padx=(8, 0))

        def clear_vehicle():
            vehicle_state["id"] = None
            vrm_var.set("")
            veh_info.config(text="No vehicle linked.", foreground="#C9A227")
            veh_clear_btn.grid_remove()
        ...
        def lookup_vehicle():
            ...
            summary = f"{result['vrm']}   {make} {model}".rstrip()
            if year:
                summary += f"  ({year})"
            set_vehicle(vehicle_db.get_by_vrm(result["vrm"]), summary)
        ...
        def sync_tagged_services():
            """Keep Tagged Item Key services in step with the products on the sale.

            A service whose Tagged Item Key matches a product's Product Group is
            auto-added with a quantity equal to the total quantity of products in
            that group, and removed when none remain. Only the lines this routine
            owns (`auto_tagged`) are touched, so manually-added services are left
            alone — and a user who deletes an auto line elsewhere isn't fought."""
            # Total product quantity per (case-folded) product group on the sale.
            group_qty = {}
            for ln in lines:
                if ln["item_type"] == "product" and (ln.get("product_group") or "").strip():
                    key = ln["product_group"].strip().lower()
                    group_qty[key] = group_qty.get(key, 0) + ln["quantity"]

            # Which services should be present, and at what quantity.
            wanted_qty, wanted_service = {}, {}
            for ln in lines:
                if ln["item_type"] != "product":
                    continue
                group = (ln.get("product_group") or "").strip()
                if not group:
                    continue
                for s in service_db.services_for_group(group):
                    wanted_qty[s["id"]] = group_qty[group.lower()]
                    wanted_service[s["id"]] = s

            # Drop auto lines no longer wanted; update or add the rest.
            lines[:] = [ln for ln in lines if not (
                ln.get("auto_tagged") and ln["service_id"] not in wanted_qty)]
            for sid, qty in wanted_qty.items():
                # Adopt any existing line for this service (not just ones we added) so
                # a service reloaded from a saved sale isn't duplicated.
                match = next((ln for ln in lines if ln["item_type"] == "service"
                              and ln["service_id"] == sid), None)
                if match:
                    match["quantity"] = qty
                    match["auto_tagged"] = True
                else:
                    s = wanted_service[sid]
                    lines.append({
                        "item_type": "service", "product_id": None,
                        "service_id": sid, "description": s["service_name"],
                        "quantity": qty, "unit_price": s["retail_price"] or 0.0,
                        "vat_rate": VAT_RATE_OPTIONS[0][1], "auto_tagged": True,
                    })

        def receive_products(basket):
            for it in basket:
                product = product_db.get_product(it["product_id"])
                lines.append({
        ...
        def add_service():
            self.open_service_picker(receive_service)

        ttk.Button(actions, text="Add Product (Ctrl+P)", command=add_product).pack(side="left")
        ttk.Button(actions, text="Add Service (Ctrl+E)", command=add_service).pack(
            side="left", padx=(8, 0))
        ...
        total_label = ttk.Label(
            self.container, text="Net 0.00   VAT 0.00   Gross 0.00   Profit 0.00",
            font=("Consolas", 10, "bold")
        )
        total_label.pack(anchor="e", pady=(6, 0))

        # Unit cost per item, cached so refresh_lines doesn't re-query each redraw.
        cost_cache = {}

        def unit_cost(ln):
            if ln["item_type"] == "product" and ln["product_id"]:
                key = ("product", ln["product_id"])
                if key not in cost_cache:
                    cost_cache[key] = product_db.average_cost(ln["product_id"])
                return cost_cache[key]
            if ln["item_type"] == "service" and ln["service_id"]:
                key = ("service", ln["service_id"])
                if key not in cost_cache:
                    svc = service_db.get_service(ln["service_id"])
                    cost_cache[key] = (svc["cost"] or 0.0) if svc else 0.0
                return cost_cache[key]
            return 0.0

        def refresh_lines():
            lines_tree.delete(*lines_tree.get_children())
            net_total = vat_total = gross_total = cost_total = 0.0
            for index, ln in enumerate(lines):
                ...
                vat_total += vat
                gross_total += gross
                cost_total += unit_cost(ln) * ln["quantity"]
                lines_tree.insert(
                    "", "end", iid=str(index),
                    values=(ln["description"], ln["item_type"].title(), ln["quantity"],
                            f"{ln['unit_price']:.2f}", f"{ln['vat_rate']:.0f}%", f"{gross:,.2f}"),
                )
            profit = net_total - cost_total
            total_label.config(
                text=f"Net {net_total:,.2f}   VAT {vat_total:,.2f}   "
                     f"Gross {gross_total:,.2f}   Profit {profit:,.2f}"
            )
        ...
        def edit_cell(event):
            ...
            def commit(_=None):
                ...
                if key == "quantity" and value == 0:  # zero qty removes the line
                    was_product = lines[index]["item_type"] == "product"
                    del lines[index]
                    if was_product:  # product change -> re-balance the auto services
                        sync_tagged_services()
                    refresh_lines()
                    self.mark_form_dirty()
                    return
                ...
                ln[key] = value
                if key == "quantity" and ln["item_type"] == "product":
                    sync_tagged_services()  # product count changed -> match auto services
                refresh_lines()
                self.mark_form_dirty()
        ...
        def delete_selected(event=None):
            ...
            if selection:
                removed = lines.pop(int(selection[0]))
                if removed["item_type"] == "product":  # re-balance auto services
                    sync_tagged_services()
                refresh_lines()
                self.mark_form_dirty()
            return "break"
        ...
        def save():
            ...
            kwargs = {"vehicle_id": vehicle_state["id"]}
            if editing:
                sale_db.update_sale(sale["id"], *args, **kwargs)
                return sale["id"]
            return sale_db.create_sale(*args, **kwargs)

        self._register_form(save=save, back=self.show_sales)

        # Keyboard shortcuts for adding lines (cleared on the next view switch).
        for seq, fn in (("<Control-p>", add_product), ("<Control-e>", add_service)):
            funcid = self.bind(seq, lambda e, f=fn: (f(), "break")[1])
        ...
        if editing:
            ...
            if sale["vehicle_id"] is not None:
                make = sale["vehicle_make"] or ""
                model = sale["vehicle_model"] or ""
                summary = f"{sale['vehicle_vrm']}   {make} {model}".rstrip()
                if sale["vehicle_year"]:
                    summary += f"  ({sale['vehicle_year']})"
                vrm_var.set(sale["vehicle_vrm"])
                set_vehicle({"id": sale["vehicle_id"]}, summary)
            for it in sale_db.get_sale_items(sale["id"]):
                ...
                lines.append({
                    ...
                    "vat_rate": it["vat_rate"], "product_group": product_group,
                })
            refresh_lines()
        ...
        if not editing:
            # A fresh sale starts with the cursor on the + (new customer) button,
            # the usual first step. Runs after _clear_container's own focus pass.
            self.after_idle(lambda: (new_customer_button["widget"]
                                     and new_customer_button["widget"].focus_set()))
